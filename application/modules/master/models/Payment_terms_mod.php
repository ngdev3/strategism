...
//THIS FUNCTION GET DATA THROUGH ID
    function getdatathroughid($table = "", $attributes = "", $id = "") {
        ...
        $CI->db->from($table);
        $CI->db->where('payment_term_id', $id);
        $CI->db->where('status', 'active');
        $query = $CI->db->get();
        if ($query->num_rows()) {
...
//  THIS FUNCTION EDIT city DATA
    function edit($id) {
        $data['payment_term_id'] = $id;//$this->input->post('state_name');
        $data['term_days'] = $this->input->post('payment_term_days');
...
//  THIS FUNCTION VIEW city DATA
    ...
    function count_city_data() {
        ...
        if (!empty($requestData['search']['value'])) {
            ...
            $this->db->where("(term_days  LIKE '%$search_val%' OR  status  LIKE '%$search_val%')");
        }
    ...
    function get_city_data($parent_id = "") {        
       ...
       //  $this->db->join($this->table.' wcntry','wct.payment_term_id=wcntry.payment_term_id','left');
         
        if (!empty($requestData['search']['value'])) {
            $search_val = $requestData['search']['value'];            
            $this->db->where("(wct.term_days LIKE '%$search_val%' OR wct.status  LIKE '%$search_val%')");
        }
        ...
        if (@$requestData['order'][0]['column'] && @$requestData['order'][0]['dir']) {
            ...
            $this->db->order_by("wct.payment_term_id", "desc");
        }
