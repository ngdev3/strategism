...
class Billing_mod extends CI_Model {
    ...
    function edit($id = null, $userdata){  
        ...
        $this->db->where('id',$id);
        $this->db->update('am_billing',$userdata);
	...
	function purchaser_details(){
		$this->db->select("purchaser_id,name");
        $this->db->from(" aa_purchaser_name");
        $this->db->where('status',"Active");
		$this->db->order_by('purchaser_id','desc');
        $query = $this->db->get();
        //echo $this->db->last_query();die;
            if ($query->num_rows() > 0) {
                return $query->result();
    ...
    function add($data){
     $this->db->insert('am_billing', $data);
	...
	function Billing_details($id){
		$this->db->select("ab.*, qual.name as quality_name, p_name.name as purchaser_name, p_name.*, s_name.name as seller_name, s_name.*, s_name.contact_person_number as cnt_number, s_name.contact_person_name as cnt_name, site_name.name as site_name, site_name.*");
		$this->db->join('aa_quality as qual', 'qual.quality_id=ab.quality','left');
		$this->db->join('aa_purchaser_name as p_name', 'p_name.purchaser_id=ab.purchaser_name','left');
	...
	function count_Billing_data() {
        ...
        if (isset($_GET['status'])) {
            ...
            $this->db->where("am_billing.status =",$_GET["status"]);
        }
		
        if (!empty($requestData['search']['value'])) {
            $search_val = $requestData['search']['value'];
            $this->db->like("(CONCAT(am_billing.truck_no))", $search_val); 
        }
		
       
		return $query = $this->db->get('am_billing');
    }
