...
class Visa_type_mod extends CI_Model {
    public $table = "aa_visa_type";
...
//  THIS FUNCTION EDIT city DATA
    function edit($id) {
        $data['visa_id'] = $id;//$this->input->post('state_name');
...
//  THIS FUNCTION VIEW city DATA
    ...
    function get_city_data($parent_id = "") {        
       ...
       //  $this->db->join($this->table.' wcntry','wct.visa_id=wcntry.visa_id','left');
         
        if (!empty($requestData['search']['value'])) {
            $search_val = $requestData['search']['value'];            
            $this->db->where("(wct.name LIKE '%$search_val%' OR wct.status  LIKE '%$search_val%')");
        }
        ...
        if (@$requestData['order'][0]['column'] && @$requestData['order'][0]['dir']) {
            ...
            $this->db->order_by("wct.visa_id", "desc");
        }
