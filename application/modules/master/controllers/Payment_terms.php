...
class Payment_terms extends CI_Controller {
    ...
    public function index() {
        ...
        $page = 'payment_terms/listing';
        $data['page'] = $page;
       // $d = array('id' => 'state_id', 'name' => 'name', 'status' => 'status');
        $data['citydata'] = $this->payment_terms_mod->getdata();
        _layout($data);
    }
    ...
    public function add() {
        ...
        if (isPostBack()) {
           // pr($_POST); die;
            $this->form_validation->set_rules('payment_term_days', 'Payment Terms Days',  'trim|required|numeric');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                set_flashdata('success', 'New Payment Terms Days added successfully');
                redirect('/master/payment_terms');
            }
        }
        
        $data['title'] = 'Track (The Rest Accounting Key) | Add Payment Terms';
        $data['page_title'] = 'Add Payment Terms';
        $data['breadcum'] = array("dashboard/" => 'Home', '' => 'Add Payment Terms');
        $page = 'payment_terms/add';        
        $d = array('table' => 'am_payment_terms','status'=>'status','column3'=>'Active');
    ...
    public function delete_city() {
        ...
        if (!empty($post)) {
            if ($this->payment_terms_mod->delete_city($post)) {                
      ...
      //  pr($_POST);die;
        ...
        if (isPostBack()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $city_name        =   $this->input->post('payment_term_days');
                $check_data         =   $this->payment_terms_mod->check_preexistance($state_id,$city_name);
                /*End of this*/
                if($check_data)
                {
                    set_flashdata('error', 'Payment Terms Days already exist.');
                    redirect("/master/payment_terms/edit/$id");
                }else{
                    $this->payment_terms_mod->edit($state_id);
                    set_flashdata('success', 'Payment Terms Days updated successfully');
                    redirect('/master/payment_terms');
                }
            }
        }
        $d = array('table' => 'am_payment_terms','status'=>'status','column3'=>'Active');
        $data['countrydata'] = getdata($d);
        $data['result'] = $this->payment_terms_mod->view($state_id);        
...
//            pr($data);die;
    ...
    public function view_all() {
        ...
        $query          =   $this->payment_terms_mod->count_city_data();
...
// pr($query->num_rows()); die;
       ...
       // $d = array('id' => 'state_id', 'name' => 'name', 'status' => 'status');
        $citydata = $this->payment_terms_mod->get_city_data(); 
        // pr($citydata);die;       
        $data   =   array();
        if(count($citydata)>0)
        {
