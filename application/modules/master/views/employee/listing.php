                ...
                <div id="mainContent">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <?=get_flashdata();?>
									...
									<a href="<?php echo base_url('master/employee/add');?>" id="back-btn" class="btn cur-p btn-primary pull-right">ADD</a>
                    ...
                    <table class="table table-striped table-bordered"  id="employee-grid-buyer">
                        <thead>
                            ...
                            <tr>
								<th width="14%">S.No.</th>
                                <th width="14%">Emp Code</th>
                                <th width="14%">Name</th>
                                <th width="14%">Designation</th>
                                <th width="14%">Department</th>
                                <th width="14%">Date of Joining</th>
...
<script>
        ...
        if(id !=''){
            ...
            jConfirm('Are you sure?', 'Confirmation Dialog', function(r) {
                if(r)
                {
                    $.ajax({
                        type:"POST",
                        url:"<?php echo site_url("master/employee/delete_city") ?>",
                         data:csrf_name+'='+csrf_token+'&id='+id,
                        success:function(data){
                            
                            window.location.href = "<?= base_url() ?>master/employee";
                        }
                    });
                }
            });
    ...
    table.dataTable({
        ...
        "ajax": {
            url: "<?php echo base_url(); ?>/master/employee/view_all?<?php echo $QUERY_STRING; ?>", // json datasource
            type: "post",
...
"orderable": false,  // set orderable for selected columns
    ...
    $(document).ready(function() {  
     ...
     $(".custom_filter").on("change",function(){ 
        ...
        if(status == 'none')
        {
           window.location.href= "<?php echo base_url('master/employee'); ?>" ;
        }
