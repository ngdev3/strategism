...
class End_clients_mod extends CI_Model {
    public $table = "am_end_clients";
...
//  THIS FUNCTION VIEW city DATA
    ...
    function count_city_data() {
        ...
        if (!empty($requestData['search']['value'])) {
            ...
            $this->db->where("(end_client_name  LIKE '%$search_val%' OR end_client_location  LIKE '%$search_val%' OR end_client_zipcode  LIKE '%$search_val%' OR mobile_no  LIKE '%$search_val%' OR contact_person  LIKE '%$search_val%' OR  status  LIKE '%$search_val%')");
        }
    ...
    function get_city_data($parent_id = "") {        
        ...
        // pr($requestData); die;
        $columns = array(
            1 => 'wct.end_client_name',
            2 => 'wct.end_client_location',
            3 => 'wct.end_client_zipcode',
            4 => 'wct.mobile_no',
            5 => 'wct.email',
            6 => 'wct.contact_person',
            7 => 'wct.status'
        );
        
        $this->db->select('wct.*');
        $this->db->from($this->table.' as wct');
       //  $this->db->join($this->table.' wcntry','wct.end_client_id=wcntry.end_client_id','left');
         
        if (!empty($requestData['search']['value'])) {
            $search_val = $requestData['search']['value'];            
            $this->db->where("(wct.end_client_name LIKE '%$search_val%' OR wct.end_client_location LIKE '%$search_val%' OR wct.end_client_zipcode LIKE '%$search_val%' OR wct.mobile_no LIKE '%$search_val%' OR wct.status  LIKE '%$search_val%')");
        }
