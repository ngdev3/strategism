...
class Skills_mod extends CI_Model {
    public $table = "aa_skills";
...
//  THIS FUNCTION EDIT city DATA
    function edit($id) {
        $data['skill_id'] = $id;//$this->input->post('state_name');
...
//  THIS FUNCTION VIEW city DATA
    ...
    function count_city_data() {
        ...
        if (!empty($requestData['search']['value'])) {
            ...
            $this->db->where("(skill_name  LIKE '%$search_val%' OR  status  LIKE '%$search_val%')");
        }
    ...
    function get_city_data($parent_id = "") {        
       ...
       //  $this->db->join($this->table.' wcntry','wct.skill_id=wcntry.skill_id','left');
         
        if (!empty($requestData['search']['value'])) {
            $search_val = $requestData['search']['value'];            
            $this->db->where("(wct.skill_name LIKE '%$search_val%' OR wct.status  LIKE '%$search_val%')");
        }
        ...
        if (@$requestData['order'][0]['column'] && @$requestData['order'][0]['dir']) {
            ...
            $this->db->order_by("wct.skill_id", "desc");
        }
    ...
    function check_preexistance($id, $city_name) {
        $this->db->select('*');
        $this->db->where('skill_id !=', $id);
        $this->db->where('skill_name ', $city_name);
